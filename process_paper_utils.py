...
def formatting_prompt(text_groups, methods, table):
    """
    Returns prompt for formatting
    """
    #TODO: one shot
    formatting_prompt = f"# Instructions\nYou will be given the methods section of a paper and a table representing animal toxicity data from a study in that paper. Your task is to identify the structure of the table in context of the study.\n 1. Using the text and the table, identify what the treatment groups are and what the measured metrics are.\n2. Observe how the table is oriented: do the treatment groups go vertically, where each column corresponds to a treatment group and each row corresponds to a metric, or do they go horizontally, where each column corresponds to a metric and each row corresponds to a treatment group? Write <vertical> if vertical and <horizontal> if horizontal. Some treatment groups derived from the paper will also be provided, so if you see any of them in the column headers, it is likely vertical and horizontal if in the row headers.\n3. If the treatment groups go vertically, are there any columns on the very left that should be combined? If so, add that to the end of the answer like so: <vertical|n>, where n is the number of columns that make up the row header, such that the data starts at column n+1. If the groups go horizontally, skip this step and answer as so: <horizontal>. Make sure not to include any extra text or whitespace besides the answer.\n# Methods\n{methods}\n# Table\n{table}\n# Groups for Reference\n{text_groups}"
...
def format_tables(pmc, text_groups, methods, tables):
    ...
    for tid, info in tables.items():
        ...
        if len(info["footnotes"]) > 0:
            ...
            for fn in info["footnotes"]:
                info_combined += fn + "\n"
        prompt = formatting_prompt(text_groups, methods, info_combined)
        answer = llama_request(prompt, 10, 0)
